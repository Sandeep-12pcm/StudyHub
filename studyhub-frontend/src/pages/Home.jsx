...
const Dashboard = () => {
  ...
  const branches = [...new Set(studyMaterials.map(m => m.branch))];
  const semesters = [...new Set(studyMaterials.map(m => m.semester))];
              ...
              <div className="p-8">
                ...
                <div className="space-y-3 mb-6">
                  ...
                  <div className={`text-xs px-4 py-2 rounded-full inline-block ${darkMode ? 'bg-slate-700 text-slate-300' : 'bg-gray-100 text-gray-600'}`}>
                    Updated: {new Date(material.dateUpdated).toLocaleDateString()}
                  </div>
                </div>
